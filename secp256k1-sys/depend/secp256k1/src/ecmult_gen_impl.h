...
static void rustsecp256k1_v0_10_0_ecmult_gen_context_clear(rustsecp256k1_v0_10_0_ecmult_gen_context *ctx) {
    ctx->built = 0;
    rustsecp256k1_v0_10_0_scalar_clear(&ctx->scalar_offset);
    rustsecp256k1_v0_10_0_ge_clear(&ctx->ge_offset);
    rustsecp256k1_v0_10_0_fe_clear(&ctx->proj_blind);
}

/* Compute the scalar (2^COMB_BITS - 1) / 2, the difference between the gn argument to
 * rustsecp256k1_v0_10_0_ecmult_gen, and the scalar whose encoding the table lookup bits are drawn
 * from (before applying blinding). */
static void rustsecp256k1_v0_10_0_ecmult_gen_scalar_diff(rustsecp256k1_v0_10_0_scalar* diff) {
    int i;

    /* Compute scalar -1/2. */
    rustsecp256k1_v0_10_0_scalar neghalf;
    rustsecp256k1_v0_10_0_scalar_half(&neghalf, &rustsecp256k1_v0_10_0_scalar_one);
    rustsecp256k1_v0_10_0_scalar_negate(&neghalf, &neghalf);

    /* Compute offset = 2^(COMB_BITS - 1). */
    *diff = rustsecp256k1_v0_10_0_scalar_one;
    for (i = 0; i < COMB_BITS - 1; ++i) {
        rustsecp256k1_v0_10_0_scalar_add(diff, diff, diff);
    }

    /* The result is the sum 2^(COMB_BITS - 1) + (-1/2). */
    rustsecp256k1_v0_10_0_scalar_add(diff, diff, &neghalf);
}
...
static void rustsecp256k1_v0_10_0_ecmult_gen(const rustsecp256k1_v0_10_0_ecmult_gen_context *ctx, rustsecp256k1_v0_10_0_gej *r, const rustsecp256k1_v0_10_0_scalar *gn) {
    ...
    rustsecp256k1_v0_10_0_fe neg;
    rustsecp256k1_v0_10_0_ge_storage adds;
    rustsecp256k1_v0_10_0_scalar d;
    ...
    rustsecp256k1_v0_10_0_scalar_add(&d, &ctx->scalar_offset, gn);
    /* Convert to recoded array. */
    for (i = 0; i < 8 && i < ((COMB_BITS + 31) >> 5); ++i) {
        recoded[i] = rustsecp256k1_v0_10_0_scalar_get_bits_limb32(&d, 32 * i, 32);
    }
    rustsecp256k1_v0_10_0_scalar_clear(&d);

    /* In rustsecp256k1_v0_10_0_ecmult_gen_prec_table we have precomputed sums of the
     ...
     * it.
     *
     * rustsecp256k1_v0_10_0_ecmult_gen_prec_table[b][index] stores the table(b, m) entries. Index
    ...
    while (1) {
        ...
        for (block = 0; block < COMB_BLOCKS; ++block) {
            ...
            rustsecp256k1_v0_10_0_fe_negate(&neg, &add.y, 1);
            rustsecp256k1_v0_10_0_fe_cmov(&add.y, &neg, sign);

            /* Add the looked up and conditionally negated value to r. */
            if (EXPECT(first, 0)) {
                /* If this is the first table lookup, we can skip addition. */
                rustsecp256k1_v0_10_0_gej_set_ge(r, &add);
                /* Give the entry a random Z coordinate to blind intermediary results. */
                rustsecp256k1_v0_10_0_gej_rescale(r, &ctx->proj_blind);
                first = 0;
            } else {
    ...
    rustsecp256k1_v0_10_0_gej_add_ge(r, r, &ctx->ge_offset);
...
static void rustsecp256k1_v0_10_0_ecmult_gen_blind(rustsecp256k1_v0_10_0_ecmult_gen_context *ctx, const unsigned char *seed32) {
    ...
    rustsecp256k1_v0_10_0_scalar diff;
    rustsecp256k1_v0_10_0_gej gb;
    rustsecp256k1_v0_10_0_fe f;
    unsigned char nonce32[32];
    rustsecp256k1_v0_10_0_rfc6979_hmac_sha256 rng;
    unsigned char keydata[64];

    /* Compute the (2^COMB_BITS - 1)/2 term once. */
    rustsecp256k1_v0_10_0_ecmult_gen_scalar_diff(&diff);

    if (seed32 == NULL) {
        /* When seed is NULL, reset the final point and blinding value. */
        rustsecp256k1_v0_10_0_ge_neg(&ctx->ge_offset, &rustsecp256k1_v0_10_0_ge_const_g);
        rustsecp256k1_v0_10_0_scalar_add(&ctx->scalar_offset, &rustsecp256k1_v0_10_0_scalar_one, &diff);
        ctx->proj_blind = rustsecp256k1_v0_10_0_fe_one;
        return;
    }
    /* The prior blinding value (if not reset) is chained forward by including it in the hash. */
    rustsecp256k1_v0_10_0_scalar_get_b32(keydata, &ctx->scalar_offset);
    ...
    rustsecp256k1_v0_10_0_fe_set_b32_mod(&f, nonce32);
    rustsecp256k1_v0_10_0_fe_cmov(&f, &rustsecp256k1_v0_10_0_fe_one, rustsecp256k1_v0_10_0_fe_normalizes_to_zero(&f));
    ctx->proj_blind = f;
