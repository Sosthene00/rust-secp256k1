...
static void musig_simple_test(void) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (i == 0) {
            ...
            CHECK(rustsecp256k1_v0_10_0_musig_nonce_gen_counter(CTX, &secnonce[i], &pubnonce[i], nonrepeating_cnt, &keypair[i], NULL, NULL, NULL) == 1);
        }
    }
...
static void pubnonce_summing_to_inf(rustsecp256k1_v0_10_0_musig_pubnonce *pubnonce) {
    ...
    rustsecp256k1_v0_10_0_musig_sum_pubnonces(CTX, summed_pubnonces, pubnonce_ptr, 2);
    CHECK(rustsecp256k1_v0_10_0_gej_is_infinity(&summed_pubnonces[0]));
    CHECK(rustsecp256k1_v0_10_0_gej_is_infinity(&summed_pubnonces[1]));
}
...
static void musig_api_tests(void) {
    ...
    CHECK(rustsecp256k1_v0_10_0_musig_nonce_gen_counter(CTX, &secnonce[0], &pubnonce[0], nonrepeating_cnt, &keypair[0], msg, &keyagg_cache, max64) == 1);
    CHECK_ILLEGAL(STATIC_CTX, rustsecp256k1_v0_10_0_musig_nonce_gen_counter(STATIC_CTX, &secnonce[0], &pubnonce[0], nonrepeating_cnt, &keypair[0], msg, &keyagg_cache, max64));
    CHECK(memcmp_and_randomize(secnonce[0].data, zeros132, sizeof(secnonce[0].data)) == 0);
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_10_0_musig_nonce_gen_counter(CTX, NULL, &pubnonce[0], nonrepeating_cnt, &keypair[0], msg, &keyagg_cache, max64));
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_10_0_musig_nonce_gen_counter(CTX, &secnonce[0], NULL, nonrepeating_cnt, &keypair[0], msg, &keyagg_cache, max64));
    CHECK(memcmp_and_randomize(secnonce[0].data, zeros132, sizeof(secnonce[0].data)) == 0);
    /* using nonce_gen_counter requires keypair */
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_10_0_musig_nonce_gen_counter(CTX, &secnonce[0], &pubnonce[0], nonrepeating_cnt, NULL, msg, &keyagg_cache, max64));
    CHECK(memcmp_and_randomize(secnonce[0].data, zeros132, sizeof(secnonce[0].data)) == 0);
    /* invalid keypair */
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_10_0_musig_nonce_gen_counter(CTX, &secnonce[0], &pubnonce[0], nonrepeating_cnt, &invalid_keypair, msg, &keyagg_cache, max64));
    CHECK(memcmp_and_randomize(secnonce[0].data, zeros132, sizeof(secnonce[0].data)) == 0);
    CHECK(rustsecp256k1_v0_10_0_musig_nonce_gen_counter(CTX, &secnonce[0], &pubnonce[0], nonrepeating_cnt, &keypair[0], NULL, &keyagg_cache, max64) == 1);
    CHECK(rustsecp256k1_v0_10_0_musig_nonce_gen_counter(CTX, &secnonce[0], &pubnonce[0], nonrepeating_cnt, &keypair[0], msg, NULL, max64) == 1);
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_10_0_musig_nonce_gen_counter(CTX, &secnonce[0], &pubnonce[0], nonrepeating_cnt, &keypair[0], msg, &invalid_keyagg_cache, max64));
    CHECK(memcmp_and_randomize(secnonce[0].data, zeros132, sizeof(secnonce[0].data)) == 0);
    CHECK(rustsecp256k1_v0_10_0_musig_nonce_gen_counter(CTX, &secnonce[0], &pubnonce[0], nonrepeating_cnt,&keypair[0], msg, &keyagg_cache, NULL) == 1);

    /* Every in-argument except nonrepeating_cnt and keypair can be NULL */
    CHECK(rustsecp256k1_v0_10_0_musig_nonce_gen_counter(CTX, &secnonce[0], &pubnonce[0], nonrepeating_cnt, &keypair[0], NULL, NULL, NULL) == 1);
    CHECK(rustsecp256k1_v0_10_0_musig_nonce_gen_counter(CTX, &secnonce[1], &pubnonce[1], nonrepeating_cnt, &keypair[1], NULL, NULL, NULL) == 1);
...
static void musig_test_vectors_noncegen(void) {
    ...
    for (i = 0; i < sizeof(vector->test_case)/sizeof(vector->test_case[0]); i++) {
        ...
        CHECK(rustsecp256k1_v0_10_0_musig_nonce_gen(CTX, &secnonce, &pubnonce, session_secrand32, sk, &pk, msg, keyagg_cache_ptr, extra_in) == 1);
...
static void musig_test_static_nonce_gen_counter(void) {
    ...
    CHECK(rustsecp256k1_v0_10_0_musig_nonce_gen_counter(CTX, &secnonce, &pubnonce, nonrepeating_cnt, &keypair, NULL, NULL, NULL) == 1);
