...
typedef struct {
    ...
    rustsecp256k1_v0_10_0_scalar* scalars;
    rustsecp256k1_v0_10_0_ge* pubkeys;
    rustsecp256k1_v0_10_0_gej* pubkeys_gej;
    rustsecp256k1_v0_10_0_scalar* seckeys;
    rustsecp256k1_v0_10_0_gej* expected_output;
    rustsecp256k1_v0_10_0_ecmult_multi_func ecmult_multi;
    ...
    /* Benchmark output. */
    rustsecp256k1_v0_10_0_gej* output;
    rustsecp256k1_v0_10_0_fe* output_xonly;
} bench_data;
...
static void bench_ecmult_teardown_helper(bench_data* data, size_t* seckey_offset, size_t* scalar_offset, size_t* scalar_gen_offset, int iters) {
    int i;
    rustsecp256k1_v0_10_0_gej sum_output, tmp;
    ...
    for (i = 0; i < iters; ++i) {
        ...
        if (scalar_gen_offset != NULL) {
            rustsecp256k1_v0_10_0_scalar_add(&sum_scalars, &sum_scalars, &data->scalars[(*scalar_gen_offset+i) % POINTS]);
        }
        if (seckey_offset != NULL) {
            rustsecp256k1_v0_10_0_scalar s = data->seckeys[(*seckey_offset+i) % POINTS];
            rustsecp256k1_v0_10_0_scalar_mul(&s, &s, &data->scalars[(*scalar_offset+i) % POINTS]);
            rustsecp256k1_v0_10_0_scalar_add(&sum_scalars, &sum_scalars, &s);
        }
    }
    rustsecp256k1_v0_10_0_ecmult_gen(&data->ctx->ecmult_gen_ctx, &tmp, &sum_scalars);
    CHECK(rustsecp256k1_v0_10_0_gej_eq_var(&tmp, &sum_output));
}
...
static void bench_ecmult_gen(void* arg, int iters) {
    ...
    for (i = 0; i < iters; ++i) {
        rustsecp256k1_v0_10_0_ecmult_gen(&data->ctx->ecmult_gen_ctx, &data->output[i], &data->scalars[(data->offset1+i) % POINTS]);
    }
}
...
static void bench_ecmult_const(void* arg, int iters) {
    ...
    for (i = 0; i < iters; ++i) {
        rustsecp256k1_v0_10_0_ecmult_const(&data->output[i], &data->pubkeys[(data->offset1+i) % POINTS], &data->scalars[(data->offset2+i) % POINTS]);
    }
}
...
static void bench_ecmult_const_xonly(void* arg, int iters) {
    ...
    for (i = 0; i < iters; ++i) {
        const rustsecp256k1_v0_10_0_ge* pubkey = &data->pubkeys[(data->offset1+i) % POINTS];
        const rustsecp256k1_v0_10_0_scalar* scalar = &data->scalars[(data->offset2+i) % POINTS];
        int known_on_curve = 1;
        rustsecp256k1_v0_10_0_ecmult_const_xonly(&data->output_xonly[i], &pubkey->x, NULL, scalar, known_on_curve);
    }
}
...
static void bench_ecmult_const_xonly_teardown(void* arg, int iters) {
    ...
    for (i = 0; i < iters; ++i) {
        const rustsecp256k1_v0_10_0_gej* pubkey_gej = &data->pubkeys_gej[(data->offset1+i) % POINTS];
        const rustsecp256k1_v0_10_0_scalar* scalar = &data->scalars[(data->offset2+i) % POINTS];
        rustsecp256k1_v0_10_0_gej expected_gej;
        rustsecp256k1_v0_10_0_ecmult(&expected_gej, pubkey_gej, scalar, NULL);
        CHECK(rustsecp256k1_v0_10_0_gej_eq_x_var(&data->output_xonly[i], &expected_gej));
    }
}
...
static void bench_ecmult_1p(void* arg, int iters) {
    ...
    for (i = 0; i < iters; ++i) {
        rustsecp256k1_v0_10_0_ecmult(&data->output[i], &data->pubkeys_gej[(data->offset1+i) % POINTS], &data->scalars[(data->offset2+i) % POINTS], NULL);
    }
}
...
static void bench_ecmult_0p_g(void* arg, int iters) {
    ...
    for (i = 0; i < iters; ++i) {
        rustsecp256k1_v0_10_0_ecmult(&data->output[i], NULL, &rustsecp256k1_v0_10_0_scalar_zero, &data->scalars[(data->offset1+i) % POINTS]);
    }
}
...
static void bench_ecmult_1p_g(void* arg, int iters) {
    ...
    for (i = 0; i < iters/2; ++i) {
        rustsecp256k1_v0_10_0_ecmult(&data->output[i], &data->pubkeys_gej[(data->offset1+i) % POINTS], &data->scalars[(data->offset2+i) % POINTS], &data->scalars[(data->offset1+i) % POINTS]);
    }
}
...
static int bench_ecmult_multi_callback(rustsecp256k1_v0_10_0_scalar* sc, rustsecp256k1_v0_10_0_ge* ge, size_t idx, void* arg) {
    bench_data* data = (bench_data*)arg;
    if (data->includes_g) ++idx;
    if (idx == 0) {
        *sc = data->scalars[data->offset1];
        *ge = rustsecp256k1_v0_10_0_ge_const_g;
    } else {
...
static void bench_ecmult_multi_teardown(void* arg, int iters) {
    ...
    for (iter = 0; iter < iters; ++iter) {
        ...
        rustsecp256k1_v0_10_0_gej_add_var(&tmp, &data->output[iter], &data->expected_output[iter], NULL);
        CHECK(rustsecp256k1_v0_10_0_gej_is_infinity(&tmp));
    }
}

static void generate_scalar(uint32_t num, rustsecp256k1_v0_10_0_scalar* scalar) {
    ...
    rustsecp256k1_v0_10_0_sha256_write(&sha256, c, sizeof(c));
    rustsecp256k1_v0_10_0_sha256_finalize(&sha256, buf);
    rustsecp256k1_v0_10_0_scalar_set_b32(scalar, buf, &overflow);
    CHECK(!overflow);
}
...
static void run_ecmult_multi_bench(bench_data* data, size_t count, int includes_g, int num_iters) {
    ...
    for (iter = 0; iter < iters; ++iter) {
        ...
        rustsecp256k1_v0_10_0_scalar total = data->scalars[(data->offset1++) % POINTS];
        size_t i = 0;
        for (i = 0; i + 1 < count; ++i) {
            rustsecp256k1_v0_10_0_scalar_mul(&tmp, &data->seckeys[(data->offset2++) % POINTS], &data->scalars[(data->offset1++) % POINTS]);
            rustsecp256k1_v0_10_0_scalar_add(&total, &total, &tmp);
        }
        rustsecp256k1_v0_10_0_scalar_negate(&total, &total);
        rustsecp256k1_v0_10_0_ecmult(&data->expected_output[iter], NULL, &rustsecp256k1_v0_10_0_scalar_zero, &total);
    }
...
int main(int argc, char **argv) {
    ...
    data.ecmult_multi = rustsecp256k1_v0_10_0_ecmult_multi_var;
    ...
    if (argc > 1) {
        if(have_flag(argc, argv, "-h")
           ...
           || have_flag(argc, argv, "help")) {
            ...
            data.ecmult_multi = rustsecp256k1_v0_10_0_ecmult_pippenger_batch_single;
        } else if(have_flag(argc, argv, "strauss_wnaf")) {
            printf("Using strauss_wnaf:\n");
            data.ecmult_multi = rustsecp256k1_v0_10_0_ecmult_strauss_batch_single;
    ...
    scratch_size = rustsecp256k1_v0_10_0_strauss_scratch_size(POINTS) + STRAUSS_SCRATCH_OBJECTS*16;
    if (!have_flag(argc, argv, "simple")) {
        data.scratch = rustsecp256k1_v0_10_0_scratch_space_create(data.ctx, scratch_size);
    } else {
        data.scratch = NULL;
    }

    /* Allocate stuff */
    data.scalars = malloc(sizeof(rustsecp256k1_v0_10_0_scalar) * POINTS);
    data.seckeys = malloc(sizeof(rustsecp256k1_v0_10_0_scalar) * POINTS);
    data.pubkeys = malloc(sizeof(rustsecp256k1_v0_10_0_ge) * POINTS);
    data.pubkeys_gej = malloc(sizeof(rustsecp256k1_v0_10_0_gej) * POINTS);
    data.expected_output = malloc(sizeof(rustsecp256k1_v0_10_0_gej) * (iters + 1));
    data.output = malloc(sizeof(rustsecp256k1_v0_10_0_gej) * (iters + 1));
    data.output_xonly = malloc(sizeof(rustsecp256k1_v0_10_0_fe) * (iters + 1));

    /* Generate a set of scalars, and private/public keypairs. */
    rustsecp256k1_v0_10_0_gej_set_ge(&data.pubkeys_gej[0], &rustsecp256k1_v0_10_0_ge_const_g);
    rustsecp256k1_v0_10_0_scalar_set_int(&data.seckeys[0], 1);
    for (i = 0; i < POINTS; ++i) {
        generate_scalar(i, &data.scalars[i]);
        if (i) {
            rustsecp256k1_v0_10_0_gej_double_var(&data.pubkeys_gej[i], &data.pubkeys_gej[i - 1], NULL);
            rustsecp256k1_v0_10_0_scalar_add(&data.seckeys[i], &data.seckeys[i - 1], &data.seckeys[i - 1]);
        }
    }
    rustsecp256k1_v0_10_0_ge_set_all_gej_var(data.pubkeys, data.pubkeys_gej, POINTS);
    ...
    * So we don't run this benchmark with low iterations to prevent slow down */
    ...
    if (data.scratch != NULL) {
        rustsecp256k1_v0_10_0_scratch_space_destroy(data.ctx, data.scratch);
    }
