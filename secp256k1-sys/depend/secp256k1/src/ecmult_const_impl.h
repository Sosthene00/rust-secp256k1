 ...
 */
static void rustsecp256k1_v0_10_0_ecmult_const_odd_multiples_table_globalz(rustsecp256k1_v0_10_0_ge *pre, rustsecp256k1_v0_10_0_fe *globalz, const rustsecp256k1_v0_10_0_gej *a) {
    rustsecp256k1_v0_10_0_fe zr[ECMULT_CONST_TABLE_SIZE];

    rustsecp256k1_v0_10_0_ecmult_odd_multiples_table(ECMULT_CONST_TABLE_SIZE, pre, zr, globalz, a);
    rustsecp256k1_v0_10_0_ge_table_set_globalz(ECMULT_CONST_TABLE_SIZE, pre, zr);
}
...
#define ECMULT_CONST_TABLE_GET_GE(r,pre,n) do { \
    ...
    rustsecp256k1_v0_10_0_fe neg_y; \
    ...
    for (m = 1; m < ECMULT_CONST_TABLE_SIZE; m++) { \
        ...
        rustsecp256k1_v0_10_0_fe_cmov(&(r)->x, &(pre)[m].x, m == index); \
        rustsecp256k1_v0_10_0_fe_cmov(&(r)->y, &(pre)[m].y, m == index); \
    } \
    (r)->infinity = 0; \
    rustsecp256k1_v0_10_0_fe_negate(&neg_y, &(r)->y, 1); \
    rustsecp256k1_v0_10_0_fe_cmov(&(r)->y, &neg_y, negative); \
} while(0)

/* For K as defined in the comment of rustsecp256k1_v0_10_0_ecmult_const, we have several precomputed
 * formulas/constants.
 * - in exhaustive test mode, we give an explicit expression to compute it at compile time: */
#ifdef EXHAUSTIVE_TEST_ORDER
static const rustsecp256k1_v0_10_0_scalar rustsecp256k1_v0_10_0_ecmult_const_K = ((SECP256K1_SCALAR_CONST(0, 0, 0, (1U << (ECMULT_CONST_BITS - 128)) - 2U, 0, 0, 0, 0) + EXHAUSTIVE_TEST_ORDER - 1U) * (1U + EXHAUSTIVE_TEST_LAMBDA)) % EXHAUSTIVE_TEST_ORDER;
/* - for the real secp256k1 group we have constants for various ECMULT_CONST_BITS values. */
#elif ECMULT_CONST_BITS == 129
/* For GROUP_SIZE = 1,3. */
static const rustsecp256k1_v0_10_0_scalar rustsecp256k1_v0_10_0_ecmult_const_K = SECP256K1_SCALAR_CONST(0xac9c52b3ul, 0x3fa3cf1ful, 0x5ad9e3fdul, 0x77ed9ba4ul, 0xa880b9fcul, 0x8ec739c2ul, 0xe0cfc810ul, 0xb51283ceul);
#elif ECMULT_CONST_BITS == 130
/* For GROUP_SIZE = 2,5. */
static const rustsecp256k1_v0_10_0_scalar rustsecp256k1_v0_10_0_ecmult_const_K = SECP256K1_SCALAR_CONST(0xa4e88a7dul, 0xcb13034eul, 0xc2bdd6bful, 0x7c118d6bul, 0x589ae848ul, 0x26ba29e4ul, 0xb5c2c1dcul, 0xde9798d9ul);
#elif ECMULT_CONST_BITS == 132
/* For GROUP_SIZE = 4,6 */
static const rustsecp256k1_v0_10_0_scalar rustsecp256k1_v0_10_0_ecmult_const_K = SECP256K1_SCALAR_CONST(0x76b1d93dul, 0x0fae3c6bul, 0x3215874bul, 0x94e93813ul, 0x7937fe0dul, 0xb66bcaaful, 0xb3749ca5ul, 0xd7b6171bul);
#else
...
static void rustsecp256k1_v0_10_0_ecmult_const(rustsecp256k1_v0_10_0_gej *r, const rustsecp256k1_v0_10_0_ge *a, const rustsecp256k1_v0_10_0_scalar *q) {
     ...
     * rustsecp256k1_v0_10_0_scalar_split_lambda function which performs such a split with the resulting s1
    ...
    static const rustsecp256k1_v0_10_0_scalar S_OFFSET = SECP256K1_SCALAR_CONST(0, 0, 0, 1, 0, 0, 0, 0);
    rustsecp256k1_v0_10_0_scalar s, v1, v2;
    rustsecp256k1_v0_10_0_ge pre_a[ECMULT_CONST_TABLE_SIZE];
    rustsecp256k1_v0_10_0_ge pre_a_lam[ECMULT_CONST_TABLE_SIZE];
    rustsecp256k1_v0_10_0_fe global_z;
    int group, i;

    /* We're allowed to be non-constant time in the point, and the code below (in particular,
     * rustsecp256k1_v0_10_0_ecmult_const_odd_multiples_table_globalz) cannot deal with infinity in a
     * constant-time manner anyway. */
    if (rustsecp256k1_v0_10_0_ge_is_infinity(a)) {
        rustsecp256k1_v0_10_0_gej_set_infinity(r);
        return;
    }

    /* Compute v1 and v2. */
    rustsecp256k1_v0_10_0_scalar_add(&s, q, &rustsecp256k1_v0_10_0_ecmult_const_K);
    rustsecp256k1_v0_10_0_scalar_half(&s, &s);
    rustsecp256k1_v0_10_0_scalar_split_lambda(&v1, &v2, &s);
    rustsecp256k1_v0_10_0_scalar_add(&v1, &v1, &S_OFFSET);
    rustsecp256k1_v0_10_0_scalar_add(&v2, &v2, &S_OFFSET);
     ...
     */
    rustsecp256k1_v0_10_0_gej_set_ge(r, a);
    rustsecp256k1_v0_10_0_ecmult_const_odd_multiples_table_globalz(pre_a, &global_z, r);
    for (i = 0; i < ECMULT_CONST_TABLE_SIZE; i++) {
        rustsecp256k1_v0_10_0_ge_mul_lambda(&pre_a_lam[i], &pre_a[i]);
    }
    ...
    for (group = ECMULT_CONST_GROUPS - 1; group >= 0; --group) {
        ...
        unsigned int bits1 = rustsecp256k1_v0_10_0_scalar_get_bits_var(&v1, group * ECMULT_CONST_GROUP_SIZE, ECMULT_CONST_GROUP_SIZE);
        unsigned int bits2 = rustsecp256k1_v0_10_0_scalar_get_bits_var(&v2, group * ECMULT_CONST_GROUP_SIZE, ECMULT_CONST_GROUP_SIZE);
        rustsecp256k1_v0_10_0_ge t;
        int j;

        ECMULT_CONST_TABLE_GET_GE(&t, pre_a, bits1);
        if (group == ECMULT_CONST_GROUPS - 1) {
            /* Directly set r in the first iteration. */
            rustsecp256k1_v0_10_0_gej_set_ge(r, &t);
        } else {
            /* Shift the result so far up. */
            for (j = 0; j < ECMULT_CONST_GROUP_SIZE; ++j) {
                rustsecp256k1_v0_10_0_gej_double(r, r);
            }
            rustsecp256k1_v0_10_0_gej_add_ge(r, r, &t);
        }
    ...
    rustsecp256k1_v0_10_0_fe_mul(&r->z, &r->z, &global_z);
}

static int rustsecp256k1_v0_10_0_ecmult_const_xonly(rustsecp256k1_v0_10_0_fe* r, const rustsecp256k1_v0_10_0_fe *n, const rustsecp256k1_v0_10_0_fe *d, const rustsecp256k1_v0_10_0_scalar *q, int known_on_curve) {
    ...
    rustsecp256k1_v0_10_0_fe g, i;
    rustsecp256k1_v0_10_0_ge p;
    rustsecp256k1_v0_10_0_gej rj;

    /* Compute g = (n^3 + B*d^3). */
    rustsecp256k1_v0_10_0_fe_sqr(&g, n);
    rustsecp256k1_v0_10_0_fe_mul(&g, &g, n);
    if (d) {
        rustsecp256k1_v0_10_0_fe b;
        VERIFY_CHECK(!rustsecp256k1_v0_10_0_fe_normalizes_to_zero(d));
        rustsecp256k1_v0_10_0_fe_sqr(&b, d);
        VERIFY_CHECK(SECP256K1_B <= 8); /* magnitude of b will be <= 8 after the next call */
        rustsecp256k1_v0_10_0_fe_mul_int(&b, SECP256K1_B);
        rustsecp256k1_v0_10_0_fe_mul(&b, &b, d);
        rustsecp256k1_v0_10_0_fe_add(&g, &b);
        if (!known_on_curve) {
             ...
             * <=> is_square(g * d)
             */
            rustsecp256k1_v0_10_0_fe c;
            rustsecp256k1_v0_10_0_fe_mul(&c, &g, d);
            if (!rustsecp256k1_v0_10_0_fe_is_square_var(&c)) return 0;
        }
    } else {
        rustsecp256k1_v0_10_0_fe_add_int(&g, SECP256K1_B);
        if (!known_on_curve) {
    ...
    rustsecp256k1_v0_10_0_fe_mul(&p.x, &g, n);
    rustsecp256k1_v0_10_0_fe_sqr(&p.y, &g);
    p.infinity = 0;

    /* Perform x-only EC multiplication of P with q. */
    VERIFY_CHECK(!rustsecp256k1_v0_10_0_scalar_is_zero(q));
    rustsecp256k1_v0_10_0_ecmult_const(&rj, &p, q);
    VERIFY_CHECK(!rustsecp256k1_v0_10_0_gej_is_infinity(&rj));
     ...
     * (X / (Z^2*d*g)). */
    rustsecp256k1_v0_10_0_fe_sqr(&i, &rj.z);
    rustsecp256k1_v0_10_0_fe_mul(&i, &i, &g);
    if (d) rustsecp256k1_v0_10_0_fe_mul(&i, &i, d);
    rustsecp256k1_v0_10_0_fe_inv(&i, &i);
    rustsecp256k1_v0_10_0_fe_mul(r, &rj.x, &i);
